...
<!-- Main content -->
      <div class="container-fluid">
		<div class="alert alert-success alert-dismissible">
                  ...
                  Data Persediaan Kain hanya dari Zone W1, W3, X1, X6
                </div>  
		<form role="form" method="post" enctype="multipart/form-data" name="form1" >  
		<div class="card card-purple">
          <div class="card-header">
            <h3 class="card-title">Filter Data Persediaan Kain</h3>
          ...
          <div class="card-body">
			...
			<div class="form-group row">
               <label for="itemno" class="col-md-1">Item</label>
               <div class="col-md-2">  
                 <input name="itemno" value="<?php echo $Item;?>" type="text" class="form-control form-control-sm">
               </div>	
            </div>
			<!--<div class="form-group row">
               <label for="itemno1" class="col-md-1">No. Item</label>
               <div class="col-md-2">  
                 <input name="itemno1" value="<?php //echo $Item1;?>" type="text" class="form-control form-control-sm">
               </div>	
            </div>-->  
			<div class="form-group row">
		  ...
		  <div class="card-footer">
			...
			<a href="pages/cetak/PersediaanKainDetailNewExcel.php?buyer=<?php echo $Buyer1; ?>&zone=<?php echo $Zone; ?>&project=<?php echo $Project; ?>&itemno=<?php echo $Item; ?>&warnano=<?php echo $NoWarna; ?>" class="btn btn-warning" target="_blank"><i class="far fa-file-excel"></i> Detail Stok</a>  
			<a href="pages/cetak/PersediaanKainExcel.php" class="btn btn-danger" target="_blank">Download Stok GKJ</a> 
			</div>	
          </div>
		  <!-- /.card-body -->
		  ...
		  <div class="card card-pink">
              <div class="card-header">
                <h3 class="card-title">Stock Kain</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
          <table id="example5" width="100%" class="table table-sm table-bordered table-striped" style="font-size:11px;">
                  <thead>
                  <tr>
	...
	if($Project!=""){				  
	$WProject= " AND bl.PROJECTCODE='$Project' " ;		
	}else{
	$WProject= " " ;	
	}
	...
	if($NoWarna!=""){
	$WNoWarna= " AND bl.DECOSUBCODE05='$NoWarna' " ;	
	}else{
	$WNoWarna= " ";	
	}
	if($Item!=""){
	$WNoItem= " AND CONCAT(trim(bl.DECOSUBCODE02),CONCAT('',trim(bl.DECOSUBCODE03)))='$Item' " ;	
	}else{
	$WNoItem= " ";	
	}	
					  
	if($Zone!=""){
	$WZone= " AND TRIM(bl.WHSLOCATIONWAREHOUSEZONECODE)='$Zone' " ;	
	}else{
	$WZone= " ";	
	}		  
					  
	if($Project=="" and $Buyer1=="" and $NoWarna=="" and $Item=="" and $Zone==""){
		$WKosong= " AND SALESORDER.CODE='' AND bl.DECOSUBCODE05='' AND CONCAT(trim(bl.DECOSUBCODE02),CONCAT('',trim(bl.DECOSUBCODE03)))='' " ;
	}				  
   $no=1;   
   $c=0;
	$sqlDB21 = " SELECT bl.*,
	...
	FROM 
		...
		AND (TRIM(b.WHSLOCATIONWAREHOUSEZONECODE)='W1' OR 
		TRIM(b.WHSLOCATIONWAREHOUSEZONECODE)='W3' OR 
		TRIM(b.WHSLOCATIONWAREHOUSEZONECODE)='X1' OR 
		TRIM(b.WHSLOCATIONWAREHOUSEZONECODE)='X6')
...
LEFT OUTER JOIN DB2ADMIN.SALESORDERDELIVERY SALESORDERDELIVERY ON
	SALESORDER.CODE = SALESORDERDELIVERY.SALESORDERLINESALESORDERCODE
WHERE
...
$rowdb29 = db2_fetch_assoc($stmt9);	
	//svr10
$mysql1=mysqli_query($con,"SELECT * FROM `tbl_keterangan` WHERE `link_kj`='".trim($rowdb21['PROJECTCODE'])." ".trim($rowdb21['LOTCODE'])." ".trim($rowdb21['WHSLOCATIONWAREHOUSEZONECODE'])." ".trim($rowdb21['WAREHOUSELOCATIONCODE'])."'");
$rMysql1=mysqli_fetch_array($mysql1);	
	?>
	  <tr>
      ...
      <td style="text-align: center"><?php echo $rowdb22['DELIVERYDATE'];?></td>
      ...
      <td style="text-align: center"><?php echo $rowdb29['GROUPING1']; ?></td>
      <td style="text-align: center"><?php echo $rowdb29['HUE1']; ?></td>
      <td style="text-align: center"><a data-pk="<?php echo trim($rowdb21['PROJECTCODE'])." ".trim($rowdb21['LOTCODE'])." ".trim($rowdb21['WHSLOCATIONWAREHOUSEZONECODE'])." ".trim($rowdb21['WAREHOUSELOCATIONCODE']);?>" data-value="<?php echo $rMysql1['keterangan']; ?>" class="keterangan_kj" href="javascipt:void(0)"><?php echo $rMysql1['keterangan']; ?></a></td>
      </tr>				  
    <?php	
		$no++;
