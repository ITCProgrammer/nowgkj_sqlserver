...
WHERE tu.status='Open' and sf.status='ok' and sf.zone='$Zone' $WhereLokasi ");
$ck1=mysqli_fetch_array($sqlCek1);
$sqlCek2=mysqli_query($con,"SELECT COUNT(*) as jml FROM	tbl_stokfull sf
LEFT JOIN tbl_upload tu ON tu.id=sf.id_upload  
WHERE tu.status='Open' and sf.status='belum cek' and sf.zone='$Zone' $WhereLokasi ");
...
LEFT JOIN tbl_upload tu ON tu.id=sf.id_upload  
WHERE tu.status='Open' AND zone='$Zone' AND SN='$Barcode' $WhereLokasi1 ");
	...
	if($Zone=="" and $Lokasi==""){
		echo"<script>alert('Zone atau Lokasi belum dipilih');</script>";
														...
														substr($Barcode,0,2)=="23" or substr($Barcode,0,2)=="24" or
													    substr($Barcode,0,3)=="000" or substr($Barcode,0,2)=="00"or
														substr($Barcode,0,2)=="80")  ){
	if($ck['jml']>0){		
	...
	BALANCE b WHERE (b.ITEMTYPECODE='KFF' OR b.ITEMTYPECODE='FKF') AND b.ELEMENTSCODE='$Barcode' ";
