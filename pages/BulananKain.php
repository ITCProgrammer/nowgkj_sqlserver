...
<!-- Main content -->
   ...
   ?>
		...
		<div class="card card-warning">
              <div class="card-header">
                <h3 class="card-title">Detail Laporan Bulanan Kain Jadi</h3>
              ...
              <div class="card-body">
			<?php	  
	$sql = mysqli_query($con," SELECT tgl_tutup,sum(rol) as qty,sum(weight) as kg FROM tbl_opname 
	WHERE DATE_FORMAT(tgl_tutup,'%Y-%m')='$TBln' GROUP BY tgl_tutup ORDER BY tgl_tutup DESC LIMIT 1 ");		  
    $r = mysqli_fetch_array($sql);
	?>			<strong>Sisa Stok Bulan Lalu : <?php echo number_format(round($r['kg'],3),3); ?></strong><br>
                <table id="example16" width="100%" class="table table-sm table-bordered table-striped" style="font-size: 11px; text-align: center;">
                  ...
                  <tr>
                    ...
                    <th colspan="4" valign="middle" style="text-align: center">Keluar</th>
                  ...
                  <tr>
                    ...
                    <th width="13%" valign="middle" style="text-align: center">Dari Packing</th>
                    <th width="13%" valign="middle" style="text-align: center">Ganti Stiker</th>
                  ...
                  <tbody>
<?php for ($i = 1; $i <= $d; $i++){ 
	$sqlMasukP = mysqli_query($con," SELECT tgl_tutup,sum(qty) as qty,sum(berat) as kg FROM tblmasukkain 
	WHERE tgl_tutup='$Thn2-$Bln2-$i' AND jns_trans='packing' GROUP BY tgl_tutup ");	
	$rMasukP = mysqli_fetch_array($sqlMasukP);
	$sqlMasukGS = mysqli_query($con," SELECT tgl_tutup,sum(qty) as qty,sum(berat) as kg FROM tblmasukkain 
	WHERE tgl_tutup='$Thn2-$Bln2-$i' AND jns_trans='ganti stiker' GROUP BY tgl_tutup ");	
	$rMasukGS = mysqli_fetch_array($sqlMasukGS);
	$sqlKeluarPK = mysqli_query($con," SELECT tgl_tutup,sum(qty) as qty,sum(berat) as kg FROM tblkeluarkain 
	WHERE tgl_tutup='$Thn2-$Bln2-$i' AND jns_trans='pengiriman' GROUP BY tgl_tutup");		  
    $rKeluarPK = mysqli_fetch_array($sqlKeluarPK);
	$sqlKeluarB = mysqli_query($con," SELECT tgl_tutup,sum(qty) as qty,sum(berat) as kg FROM tblkeluarkain 
	WHERE tgl_tutup='$Thn2-$Bln2-$i' AND jns_trans='bongkaran' GROUP BY tgl_tutup");		  
    $rKeluarB = mysqli_fetch_array($sqlKeluarB);
	$sqlKeluarPS = mysqli_query($con," SELECT tgl_tutup,sum(qty) as qty,sum(berat) as kg FROM tblkeluarkain 
	WHERE tgl_tutup='$Thn2-$Bln2-$i' AND jns_trans='potong sample' GROUP BY tgl_tutup");		  
    $rKeluarPS = mysqli_fetch_array($sqlKeluarPS);
	$sqlKeluarPQ = mysqli_query($con," SELECT tgl_tutup,sum(qty) as qty,sum(berat) as kg FROM tblkeluarkain 
	WHERE tgl_tutup='$Thn2-$Bln2-$i' AND jns_trans='pass qty' GROUP BY tgl_tutup");		  
    $rKeluarPQ = mysqli_fetch_array($sqlKeluarPQ);
	if($i=="1"){
	$sisa+=round($r['kg'],2)+((round($rMasukP['kg'],3)+round($rMasukGS['kg'],3))-(round($rKeluarPK['kg'],3)+round($rKeluarB['kg'],3)+round($rKeluarPS['kg'],3)+round($rKeluarPQ['kg'],3)));	
	}else{
	$sisa+=((round($rMasukP['kg'],3)+round($rMasukGS['kg'],3))-(round($rKeluarPK['kg'],3)+round($rKeluarB['kg'],3)+round($rKeluarPS['kg'],3)+round($rKeluarPQ['kg'],3)));
	}
	
?>
	  <tr>
	  <td><?php echo $i; ?></td>
	  <td align="right"><?php echo number_format(round($rMasukP['kg'],2),2); ?></td>
	  <td align="right"><?php echo number_format(round($rMasukGS['kg'],2),2); ?></td>
	  <td align="right"><?php echo number_format(round($rKeluarPK['kg'],2),2); ?></td>
	  <td align="right"><?php echo number_format(round($rKeluarB['kg'],2),2); ?></td>
	  <td align="right"><?php echo number_format(round($rKeluarPS['kg'],2),2); ?></td>
	  <td align="right"><?php echo number_format(round($rKeluarPQ['kg'],2),2); ?></td>
	  <td align="right"><strong><?php echo number_format(round($sisa,3),3); ?></strong></td>
      </tr>
	  				  
	<?php 
	 $no++; 
	$tMP+=round($rMasukP['kg'],3);
	$tMGS+=round($rMasukGS['kg'],3);
	$tPK+=round($rKeluarPK['kg'],3);
	$tB+=round($rKeluarB['kg'],3);
	$tPS+=round($rKeluarPS['kg'],3);
	$tPQ+=round($rKeluarPQ['kg'],3);
	} 
	$tS=(round($r['kg'],2)+$tMP+$tMGS)-($tPK+$tB+$tPS+$tPQ);
					  ?>
				  </tbody>
                  <tfoot>
	 <tr>
	   <td><strong>Total</strong></td>
	    <td align="right"><strong><?php echo number_format(round($tMP,2),2); ?></strong></td>
	    <td align="right"><strong><?php echo number_format(round($tMGS,2),2); ?></strong></td>
	    <td align="right"><strong><?php echo number_format(round($tPK,2),2); ?></strong></td>
	    <td align="right"><strong><?php echo number_format(round($tB,2),2); ?></strong></td>
	    <td align="right"><strong><?php echo number_format(round($tPS,2),2); ?></strong></td>
