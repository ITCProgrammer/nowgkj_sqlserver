...
FROM
	tbl_opname
WHERE
	tgl_tutup = DATE_FORMAT(NOW(), '%Y-%m-%d')
LIMIT 1");
...
if($dcek['ck']>0){
    ...
    so.ORDPRNCUSTOMERSUPPLIERCODE,
    akj.LEGALNAME1,
    akj.ORDERPARTNERBRANDCODE,
    akj.LONGDESCRIPTION
FROM (
    SELECT 
        SUM(b.BASEPRIMARYQUANTITYUNIT) AS BERAT,
        SUM(b.BASESECONDARYQUANTITYUNIT) AS YD,
        COUNT(b.LOTCODE) AS ROLL,
        b.LOTCODE,
        b.PROJECTCODE,
        b.ITEMTYPECODE,
        b.DECOSUBCODE01,
        ...
        FROM (
            SELECT
                ...
                ROW_NUMBER() OVER (
                    ...
                    ORDER BY CREATIONDATETIME DESC, TRANSACTIONDETAILNUMBER DESC
                ) AS RN
            FROM STOCKTRANSACTION
            WHERE ITEMTYPECODE IN ('FKF','KFF')
              AND LOGICALWAREHOUSECODE = 'M031'
        ) tmp
        WHERE RN = 1
    ) t ON b.ELEMENTSCODE = t.ITEMELEMENTCODE
    LEFT JOIN QUALITYREASON q1 
        ON t.QUALITYREASONCODE = q1.CODE
    LEFT JOIN PRODUCT p 
        ON p.ITEMTYPECODE = b.ITEMTYPECODE
    ...
    WHERE b.ITEMTYPECODE IN ('FKF','KFF')
      AND b.LOGICALWAREHOUSECODE = 'M031'
      AND b.WHSLOCATIONWAREHOUSEZONECODE IN ('W1','W2','W3','X1','X2','X6','X7')
    GROUP BY 
        b.ITEMTYPECODE,
        b.DECOSUBCODE01,b.DECOSUBCODE02,b.DECOSUBCODE03,
        b.DECOSUBCODE04,b.DECOSUBCODE05,b.DECOSUBCODE06,
        b.DECOSUBCODE07,b.DECOSUBCODE08,
        b.PROJECTCODE,b.LOTCODE,
        b.BASEPRIMARYUNITCODE,b.BASESECONDARYUNITCODE,
        b.WHSLOCATIONWAREHOUSEZONECODE,b.WAREHOUSELOCATIONCODE,
        p.LONGDESCRIPTION,t.QUALITYREASONCODE,q1.LONGDESCRIPTION
) bl
LEFT JOIN DB2ADMIN.SALESORDER so 
    ON bl.PROJECTCODE = so.CODE
LEFT JOIN DB2ADMIN.ITXVIEWAKJ akj 
    ON so.CODE = akj.CODE;
		";
	...
	if ($rowdb21['ITEMTYPECODE'] === "KFF") {
		$jns = "KAIN";
	} elseif ($rowdb21['ITEMTYPECODE'] === "FKF") {
		$jns = "KRAH";
	}
