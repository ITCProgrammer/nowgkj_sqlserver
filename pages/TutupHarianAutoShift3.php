...
FROM
	tbl_opname_shift
WHERE
	tgl_tutup = DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 DAY), '%Y-%m-%d') AND shift = '3'
LIMIT 1");
$dcek=mysqli_fetch_array($cektgl);
if($dcek['ck']>0){
	echo "<script>";
	echo "alert('Stok Tgl ".$dcek['tgl_tutup']." Shift 3 Ini Sudah Pernah ditutup')";
	echo "</script>";
}else if($_GET['note']!="" or $_GET['note']=="Berhasil"){
	echo "Tutup Transaksi Berhasil Shift 3";
}else{
...
b.WHSLOCATIONWAREHOUSEZONECODE,b.WAREHOUSELOCATIONCODE,p.LONGDESCRIPTION) bl LEFT OUTER JOIN
	...
	shift = '3',
	tgl_tutup = DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 DAY), '%Y-%m-%d'),
	tgl_buat =now()") or die("GAGAL SIMPAN");	
	
	}
	if($simpan){		
        echo "<meta http-equiv='refresh' content='0; url=TutupHarianAutoShift3.php?note=Berhasil'>";		
		}	
}
