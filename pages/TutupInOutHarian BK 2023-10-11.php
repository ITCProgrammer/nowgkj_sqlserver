...
s.LOTCODE, SUM(s.BASEPRIMARYQUANTITY) AS KG, s.ITEMELEMENTCODE, COUNT(s.ITEMELEMENTCODE) AS JML, s.PROJECTCODE , s1.PROJECTCODE AS PRO_DESN, s1.LOTCODE AS LOT_DESN
FROM STOCKTRANSACTION s
LEFT OUTER JOIN (
SELECT  
s.LOTCODE, s.PROJECTCODE ,s.TRANSACTIONNUMBER  
FROM STOCKTRANSACTION s
WHERE s.ITEMTYPECODE='KFF' AND s.LOGICALWAREHOUSECODE ='M031' AND
s.TEMPLATECODE = '314' 
GROUP BY s.LOTCODE, s.PROJECTCODE ,s.TRANSACTIONNUMBER
) s1 ON s.TRANSACTIONNUMBER = s1.TRANSACTIONNUMBER
WHERE s.ITEMTYPECODE='KFF' AND s.LOGICALWAREHOUSECODE ='M031' AND
s.TEMPLATECODE = '313' AND s.TRANSACTIONDATE = '$Awal' 
...
s.LOTCODE,s.CREATIONUSER, s.PROJECTCODE, s.ITEMELEMENTCODE, s1.PROJECTCODE, s1.LOTCODE";
	$stmt1GS   = db2_exec($conn1,$sqlDB21GS, array('cursor'=>DB2_SCROLLABLE));
	//}				  
    while($rowdb21GS = db2_fetch_assoc($stmt1GS)){
	$kdbenangGS=trim($rowdb21GS['DECOSUBCODE01'])." ".trim($rowdb21GS['DECOSUBCODE02'])." ".trim($rowdb21GS['DECOSUBCODE03'])." ".trim($rowdb21GS['DECOSUBCODE04'])." ".trim($rowdb21GS['DECOSUBCODE05'])." ".trim($rowdb21GS['DECOSUBCODE06'])." ".trim($rowdb21GS['DECOSUBCODE07'])." ".trim($rowdb21GS['DECOSUBCODE08']);
		
$simpanMGS=mysqli_query($con,"INSERT INTO `tblmasukkain` SET 
tgl_masuk	= '".substr($rowdb21GS['TRANSACTIONDATE'],0,10)."',
qty			= '".$rowdb21GS['JML']."',
berat		= '".round($rowdb21GS['KG'],2)."',
tipe		= '".$tipeGS."',
element		= '".$rowdb21GS['ITEMELEMENTCODE']."',
no_order	= '".$rowdb21GS['PRO_DESN']."',
lot			= '".$rowdb21GS['LOT_DESN']."',
tgl_tutup 	= '".$Awal."',
jns_trans	= 'ganti stiker',
tgl_buat 	= now()") or die("GAGAL SIMPAN MASUK KAIN GS");		

}	
				
	//Pengiriman Kain
				
$sqlDB21PK = " SELECT SALESDOCUMENT.GOODSISSUEDATE, ITXVIEWLAPKIRIMPPC.PROVISIONALCODE,
	   ...
	   WHERE SALESDOCUMENT.GOODSISSUEDATE = '$Awal' AND SALESDOCUMENT.PROGRESSSTATUS='2' 
	   AND NOT ITXVIEWLAPKIRIMPPC.CODE IS NULL";
		...
		WHERE SALESORDER.CODE='$projectPK' AND SALESORDERLINE.ORDERLINE='" . $rowdb21PK['DLVSALESORDERLINEORDERLINE'] . "'";
	...
	FROM ITXVIEW_ALLOCATION_SURATJALAN_PPC 
		 WHERE ITXVIEW_ALLOCATION_SURATJALAN_PPC.CODE='".$rowdb21PK['CODE']."' 
...
tgl_keluar 	= '".substr($rowdb21PK['GOODSISSUEDATE'], 0, 10)."',
qty 		= '".$rowdb23PK['ROL']."',
...
FROM STOCKTRANSACTION s
WHERE s.ITEMTYPECODE='KFF' AND s.LOGICALWAREHOUSECODE ='M031' AND
s.TEMPLATECODE = '120' AND s.TRANSACTIONDATE = '$Awal' 
