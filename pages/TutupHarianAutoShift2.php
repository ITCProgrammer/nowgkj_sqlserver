...
FROM
	tbl_opname_shift
WHERE
	tgl_tutup = DATE_FORMAT(NOW(), '%Y-%m-%d') AND shift = '2'
LIMIT 1");
$dcek=mysqli_fetch_array($cektgl);
if($dcek['ck']>0){
	echo "<script>";
	echo "alert('Stok Tgl ".$dcek['tgl_tutup']." Shift 2 Ini Sudah Pernah ditutup')";
	echo "</script>";
}else if($_GET['note']!="" or $_GET['note']=="Berhasil"){
	echo "Tutup Transaksi Berhasil Shift 2";
}else{
...
b.WHSLOCATIONWAREHOUSEZONECODE,b.WAREHOUSELOCATIONCODE,p.LONGDESCRIPTION) bl LEFT OUTER JOIN
	...
	if($simpan){		
        echo "<meta http-equiv='refresh' content='0; url=TutupHarianAutoShift2.php?note=Berhasil'>";		
		}	
}
