...
header("Content-Disposition: attachment; filename=persediaanBalanceZoneW3F.xls");//ganti nama sesuai keperluan
...
<table id="" class="">
   ...
   $c=0;
	...
	//}else{
	$sqlDB21 = " SELECT * FROM 
	BALANCE b WHERE (b.ITEMTYPECODE='KFF' OR b.ITEMTYPECODE='FKF') AND b.LOGICALWAREHOUSECODE='M031' AND WHSLOCATIONWAREHOUSEZONECODE LIKE 'W3%' AND WAREHOUSELOCATIONCODE LIKE 'F%' ";
...
WHERE SN='$rowdb21[ELEMENTSCODE]' ");
	...
	if(trim($rowdb26['ITEMDESCRIPTION_YND'])!=""){$jskain=$rowdb26['ITEMDESCRIPTION_YND'];}else{ $jskain=$rQC['jenis_kain']; }
