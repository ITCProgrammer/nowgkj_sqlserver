<?php
	...
	header("Content-Disposition: attachment; filename=persediaankainjadi2022Detail11".$_GET['tgl'].".xls"); //ganti nama sesuai keperluan
...
<?php	$no++;
		...
		if(trim($rowdb21['satuan_len'])=="yd"){$tyd=$rowdb21['length'];}else{$tyd=0;}
		$totyd=$totyd+$tyd;
		if(trim($rowdb21['satuan_len'])=="m"){$tmtr=$rowdb21['length'];}else{$tmtr=0;}
		$totmtr=$totmtr+$tmtr;
