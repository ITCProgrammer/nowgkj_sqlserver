...
header("Content-Disposition: attachment; filename=persediaanBalanceItemNEW_".$_GET['itemno']."_".$_GET['warnano'].".xls");//ganti nama sesuai keperluan
...
<table id="example1" class="table table-sm table-bordered table-striped" style="font-size:13px;">
   ...
   $c=0;
	$sqlDB21 = " SELECT bl.*,SALESORDER.CODE, SALESORDER.EXTERNALREFERENCE, SALESORDER.ORDPRNCUSTOMERSUPPLIERCODE,
		...
		TRIM(b.WHSLOCATIONWAREHOUSEZONECODE) LIKE '%X%')) bl LEFT OUTER JOIN
...
LEFT JOIN
    ...
    LEFT JOIN ORDERPARTNER ORDERPARTNER ON
        BUSINESSPARTNER.NUMBERID = ORDERPARTNER.ORDERBUSINESSPARTNERNUMBERID) D
ON
...
LEFT JOIN (
	...
	WHERE PRODUCTIONDEMANDSTEP.PRODUCTIONORDERCODE='$rowdb21[LOTCODE]' LIMIT 1) F ON
    PRODUCTIONORDER.CODE = F.PRODUCTIONORDERCODE
WHERE PRODUCTIONORDER.CODE='$rowdb21[LOTCODE]'   
GROUP BY
    PRODUCTIONORDER.CODE,
    PRODUCTIONRESERVATION.PROJECTCODE,
    PRODUCTIONRESERVATION.ORDERCODE,
...
LIMIT 1 ";
	...
	$POno=$rQC['no_po'];	
	if($langganan!=""){$langganan1=$langganan;}else if($cust!=""){$langganan1=$cust;}else{$langganan1=$rowdb30['LEGALNAME1'];}	
	if($buyer!=""){$buyer1=$buyer;}else if($byr!=""){$buyer1=$byr;}else{$buyer1=$rowdb30['BUYER'];}	
	if($item!=""){$item1=$item;}else if($itemLama!=""){$item1=$itemLama;}else{$item1="";}	
	if($PO!=""){$PO1=$PO;}else if($POno!=""){$PO1=$POno;}else{$PO1=$rowdb30['PO_NUMBER'];}
	if(trim($rowdb21['PROJECTCODE'])!=""){$Projct=$rowdb21['PROJECTCODE'];}else{$Projct=$rQC['no_order'];}	
?>
	  <tr>
      ...
      <td style="text-align: center"><?php echo $item1; ?></td>
      <td style="text-align: left"><?php echo $langganan1; ?></td>
      <td style="text-align: left"><?php echo $buyer1; ?></td>
      <td style="text-align: left"><?php echo $PO1; ?></td>
      <td style="text-align: center"><?php echo $Projct; ?></td>
