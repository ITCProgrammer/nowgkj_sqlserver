<?php
	...
	header("Content-Disposition: attachment; filename=MasukKainNew7PerBulanSTK.xls"); //ganti nama sesuai keperluan
...
FROM
	STOCKTRANSACTION
	LEFT OUTER JOIN (
	SELECT * FROM (SELECT ITEMELEMENTCODE,QUALITYREASONCODE, CREATIONDATETIME,
        ROW_NUMBER() OVER (PARTITION BY ITEMELEMENTCODE ORDER BY CREATIONDATETIME DESC) AS RN FROM STOCKTRANSACTION	WHERE 
	(ITEMTYPECODE = 'FKF'
		OR ITEMTYPECODE = 'KFF')
	AND 
	LOGICALWAREHOUSECODE = 'M031') T
	INNER JOIN BALANCE B ON B.ELEMENTSCODE = T.ITEMELEMENTCODE 
	WHERE T.RN='1'
	) BALANCE ON BALANCE.ELEMENTSCODE = STOCKTRANSACTION.ITEMELEMENTCODE 
	LEFT OUTER JOIN QUALITYREASON ON STOCKTRANSACTION.QUALITYREASONCODE = QUALITYREASON.CODE
	LEFT OUTER JOIN QUALITYREASON Q1 ON BALANCE.QUALITYREASONCODE = Q1.CODE
WHERE
