...
header("Content-Disposition: attachment; filename=persediaanBalanceZoneW3FM_FW.xls");//ganti nama sesuai keperluan
...
<table id="" class="">
   ...
   $c=0;
	...
	//}else{
	$sqlDB21 = " SELECT * FROM 
	BALANCE b WHERE (b.ITEMTYPECODE='KFF' OR b.ITEMTYPECODE='FKF') AND b.LOGICALWAREHOUSECODE='M031' AND WHSLOCATIONWAREHOUSEZONECODE LIKE 'W3%' AND  
	(WAREHOUSELOCATIONCODE LIKE 'FM%' OR 
	WAREHOUSELOCATIONCODE LIKE 'FN%' OR
	WAREHOUSELOCATIONCODE LIKE 'FO%' OR
	WAREHOUSELOCATIONCODE LIKE 'FP%' OR
	WAREHOUSELOCATIONCODE LIKE 'FQ%' OR 
	WAREHOUSELOCATIONCODE LIKE 'FR%' OR
	WAREHOUSELOCATIONCODE LIKE 'FS%' OR
	WAREHOUSELOCATIONCODE LIKE 'FT%' OR
	WAREHOUSELOCATIONCODE LIKE 'FU%' OR
	WAREHOUSELOCATIONCODE LIKE 'FV%' OR
	WAREHOUSELOCATIONCODE LIKE 'FW%') ";
	...
	$sqlDB24 = " SELECT STOCKTRANSACTION.TRANSACTIONDATE, STOCKTRANSACTION.QUALITYREASONCODE,STOCKTRANSACTION.ITEMELEMENTCODE,
    	STOCKTRANSACTION.PROJECTCODE,QUALITYREASON.LONGDESCRIPTION 
		FROM DB2ADMIN.STOCKTRANSACTION STOCKTRANSACTION LEFT OUTER JOIN 
      	DB2ADMIN.QUALITYREASON QUALITYREASON ON 
      	STOCKTRANSACTION.QUALITYREASONCODE=QUALITYREASON.CODE 
		WHERE STOCKTRANSACTION.ITEMELEMENTCODE='$rowdb21[ELEMENTSCODE]' ORDER BY STOCKTRANSACTION.TRANSACTIONDATE DESC	LIMIT 1 ";	
   ...
   FROM DB2ADMIN.ORDERITEMORDERPARTNERLINK ORDERITEMORDERPARTNERLINK WHERE
	...
	$sqlDB26 = " SELECT SALESORDERLINE.ITEMDESCRIPTION,
       SALESORDERLINE.EXTERNALREFERENCE 
       FROM DB2ADMIN.SALESORDERLINE WHERE
       SALESORDERLINE.ITEMTYPEAFICODE='$rowdb21[ITEMTYPECODE]' AND	   
	   (SALESORDERLINE.PROJECTCODE='$rowdb21[PROJECTCODE]' OR SALESORDERLINE.SALESORDERCODE='$rowdb21[PROJECTCODE]') AND
	   SALESORDERLINE.SUBCODE01='$rowdb21[DECOSUBCODE01]' AND
       SALESORDERLINE.SUBCODE02='$rowdb21[DECOSUBCODE02]' AND
       SALESORDERLINE.SUBCODE03='$rowdb21[DECOSUBCODE03]' AND
	   SALESORDERLINE.SUBCODE04='$rowdb21[DECOSUBCODE04]' AND
	...
	TEMPLATECODE ='304' AND (ITEMTYPECODE='FKF' OR ITEMTYPECODE='KFF') AND LOGICALWAREHOUSECODE ='M031' ORDER BY TRANSACTIONDATE ASC LIMIT 1 ";
...
WHERE SN='$rowdb21[ELEMENTSCODE]' ");
	...
	if(trim($rowdb26['ITEMDESCRIPTION'])!=""){$jskain=$rowdb26['ITEMDESCRIPTION'];}else{ $jskain=$rQC['jenis_kain']; }
	$pos1=strpos($jskain,"RECYCLED");	
	$pos2=strpos($jskain,"ORGANIC");	
	if($pos1>0){ $ket="RECYCLED";}else if($pos1>2){$ket="ORGANIC";}	else{$ket="";}
?>
	  <tr>
