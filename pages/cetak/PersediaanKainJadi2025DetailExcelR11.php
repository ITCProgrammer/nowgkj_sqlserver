...
header("Content-Disposition: attachment; filename=persediaankainjadi202511" . $_GET['tgl'] . ".xls"); //ganti nama sesuai keperluan
...
";
  ...
  ?>
    <tr>
      <td><?php if ($tglMutasi != "0000-00-00") {
            echo $tglMutasi;
          } else {
          } ?></td>
  ...
  <?php $no++;
    ...
    if (trim($rowdb21['satuan_len']) == "yd") {
      $tyd = $rowdb21['length'];
    } else {
      $tyd = 0;
    }
    $totyd = $totyd + $tyd;
    if (trim($rowdb21['satuan_len']) == "m") {
      $tmtr = $rowdb21['length'];
    } else {
      $tmtr = 0;
    }
